...
int main()
{
	...
	Mat gray_photo(img.size(), img.type());
	Gray_Filter(img, gray_photo);

	imwrite("gray_img.jpg", gray_photo);

	Mat gauss_filter(gray_photo.size(), gray_photo.type());
	gauss_filter_gray(gray_photo, gauss_filter, 1, 7);

	imwrite("gray_gauss.jpg", gauss_filter);

	Mat sobel_x(gauss_filter.size(), gauss_filter.type());
	Mat sobel_y(gauss_filter.size(), gauss_filter.type());
	sobel_operator_gray(gauss_filter, sobel_x, sobel_y);

	imwrite("gray_sobel_x.jpg", sobel_x);
	imwrite("gray_sobel_y.jpg", sobel_y);

	Mat suppression(gauss_filter.size(), gauss_filter.type());
	suppression_of_non_maxima(sobel_x, sobel_y, gauss_filter, suppression);

	imwrite("photo_sup.jpg", suppression);

	Mat Kenny(suppression.size(), suppression.type());
	double_threshold_filtering(suppression, Kenny, 180, 74);

	imwrite("photoKenny_algo.jpg", Kenny);

	Mat img2 = imread("example.jpg");
	int countL;
	search_line(img2, 0.1, &countL);

	Mat bin_photo(img2.size(), CV_8UC1);
	Mat img2_;
	img2.copyTo(img2_);
	Canny(img2, bin_photo, 50, 200, 3);
	vector<Vec2f> lines;
	...
	for (size_t i = 0; i < lines.size(); i++)
	{
		...
		line(img2_, pt1, pt2, Scalar(0, 255, 0), 3, LINE_AA);
	}
	imwrite("line_OpenCV.jpg", img2_);

	std::cout << "count objects on image: " << countL << std::endl;
